...
[![Video Duplicate Finder logo](https://images.weserv.nl/?url=raw.githubusercontent.com/jlesage/docker-templates/master/jlesage/images/video-duplicate-finder-icon.png&w=200)](https://github.com/0x90d/videoduplicatefinder)[![Video Duplicate Finder](https://images.placeholders.dev/?width=704&height=110&fontFamily=Georgia,sans-serif&fontWeight=400&fontSize=52&text=Video%20Duplicate%20Finder&bgColor=rgba(0,0,0,0.0)&textColor=rgba(121,121,121,1))](https://github.com/0x90d/videoduplicatefinder)
...
```
docker stop video-duplicate-finder
```
  2. Remove the container:
```
docker rm video-duplicate-finder
```
...
services:
  video-duplicate-finder:
    ...
    ports:
      - "5800:5800"
    volumes:
      - "/docker/appdata/video-duplicate-finder:/config:rw"
      - "$HOME:/storage:rw"
```
...
```
docker pull jlesage/video-duplicate-finder
```
...
container image.
  ...
  3.  In the search bar, type the name of the container (`jlesage/video-duplicate-finder`).
  4.  Select the image, click *Download* and then choose the `latest` tag.
  5.  Wait for the download to complete.  A  notification will appear once done.
  6.  Click on *Container* in the left pane.
  7.  Select your Video Duplicate Finder container.
...
`video-duplicate-finder.domain.tld` to the container at `127.0.0.1:5800`.
...
upstream docker-video-duplicate-finder {
...
`server.domain.tld/video-duplicate-finder` to the container at `127.0.0.1:5800`.
...
server {
	...
	location = /video-duplicate-finder {return 301 $scheme://$http_host/video-duplicate-finder/;}
	location /video-duplicate-finder/ {
		proxy_pass http://docker-video-duplicate-finder/;
